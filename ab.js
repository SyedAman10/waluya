...
async function processMultipleConversations(conversationIds) {
  ...
  // Log email results summary
  const clientEmailsSent = results.filter(r => r.emailResults?.clientEmail?.success).length;
  const teamEmailsSent = results.filter(r => r.emailResults?.teamEmail?.success).length;
  console.log(`Client emails sent: ${clientEmailsSent}`);
  console.log(`Team emails sent: ${teamEmailsSent}`);
}
